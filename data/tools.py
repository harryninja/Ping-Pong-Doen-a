...
import shutil
import random
'''isso aqui é uma coletanea de onde ta tudo que vai usar são caminhos para as imagens, sons, e fontes'''
def clean_files():
    '''remove os pyc no pycache'''
...
class States:
    ...
    def select_option(self, i):
        '''seleciona no menu via mouse ou teclado'''
    ...
    def change_selected_option(self, op=0):
        '''troca o que o mouse passar por cima'''
        for i,opt in enumerate(self.rendered["des"]):
            if opt[1].collidepoint(pg.mouse.get_pos()):
                self.selected_index = i
